...
def create_llm_service() -> LLMServiceInterface:
    """
    Composition root — wires the correct LLM service based on config.USE_LOCAL.
    """
    if config.USE_LOCAL:
        from infra.llm_service_local import LlamaVisionService
        return LlamaVisionService(model_id=config.OLLAMA_MODEL_ID)
    else:
        from infra.llm_service import LlamaVisionService
        return LlamaVisionService(
...
class StyleFinderApp:
    ...
    def __init__(self, dataset_path: str, serp_api_key: str | None = None):
        """
        ...
        self.llm_service: LLMServiceInterface = create_llm_service()
...
def create_gradio_interface(app: StyleFinderApp) -> gr.Blocks:
    ...
    """
    with gr.Blocks(theme=Soft(), title="Fashion Style Analyzer") as demo:
        gr.Markdown(
            """
        ...
        with gr.Row():
            gr.Image(value="examples/test-6.png", label="Example 1", show_label=True, scale=1)
            gr.Image(value="examples/test-7.png", label="Example 2", show_label=True, scale=1)
