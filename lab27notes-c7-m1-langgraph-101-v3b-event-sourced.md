...
**AgentLedger (agent observability infrastructure, in flight).** The AgentLedger concept (.NET Aspire + Marten + Wolverine + Python SDK) sits at the same architectural seam V3b's substrate occupies: an event log around an agent's runtime, not part of the agent itself. The Python-SDK side of AgentLedger reproduces V3b's translator + per-run streaming consumer shape at a different language layer; the .NET side reproduces the event store + projection pattern using Marten's event-sourcing primitives. V3b is the courseware-scale rehearsal of the patterns AgentLedger productionises. The `EventTranslatingStreamConsumer` decorator-over-inner pattern in particular maps directly to AgentLedger's "instrument any agent without modifying it" claim.
