...
| Course 6 - Module 2 | YouTube Tool-Calling Agent | [course6-module2-lab3/README.md](course6-module2-lab3/README.md) | Three orchestration strategies (manual `while`-loop, two-step LCEL chain, recursive LCEL chain) behind one `VideoAgentInterface`, `AgentStrategy` enum with one-line swap in container, per-external-service interfaces (`TranscriptClient`, `YouTubeSearchClient`, `YouTubeMetadataClient`), tool factories with closure-captured dependencies, infra-raises-application-translates error boundary, yt-dlp typing containment to one file, 91 tests (89 unit + 2 `@pytest.mark.slow` concurrency integration), max-iteration cap as production backstop, structured unknown-tool-name error enables LLM self-correction, `isinstance(content, str)` guard against multimodal content blocks, JSON serialisation at tool boundary, `ThreadPoolExecutor` timeout wrappers, `MetadataResult` TypedDict honest about None fields |
| Course 6 - Module 3 | Data Visualisation Agent | [lab25notes-c6-m3-data-viz-agent.md](lab25notes-c6-m3-data-viz-agent.md) | LangChain 1.x `create_agent` modernisation of IBM's `create_pandas_dataframe_agent`, explicit `save_figure(fig)` primitive replacing implicit `plt.get_fignums()` capture, diagnostic discipline (instrument before redesign), onion architecture making the redesign cheap (two files changed, interfaces absorbed it), `AgentTrace` domain object pairing tool calls and results by `tool_call_id`, `LocalFigureStore` with resume-from-disk for dev UX, schema grounding leakage into the answer space (system prompt facts surface without tool calls), `matplotlib.use("Agg")` at module load as tool-boundary enforcement, two LLM calls per tool-using turn as floor, 51-test suite running without API key |
...
- Wall-clock timeouts on every external call, not just retries. `socket_timeout` where the library exposes it; `ThreadPoolExecutor` wrapper where it doesn't. A hung network call with no timeout blocks the thread indefinitely — in a concurrent service, one hang starves the pool. `max_iterations` bounds LLM call count; nothing bounds wall-clock time per tool call without an explicit timeout.

**After Data Visualisation Agent lab (Course 6 - Module 3):**
- LLM behaviour rules belong at the tool boundary, not the prompt. *"Do not call plt.show()"* in the system prompt held most of the time and broke occasionally — GPT-4.1-mini's training-data prior on `plt.show()` is too strong for a prompt rule to fully override. `matplotlib.use("Agg")` at module load is the actual defence: makes `plt.show()` a no-op regardless of what the LLM emits. Prompts shift probabilities; tool boundaries enforce.
- Two LLM calls per tool-using turn is the floor; more is the norm. Decide → tool → summarise. The two-figure Dalc/Walc query went three deep: decide → save first → decide again → save second → summarise. Each round-trip carries the full growing message history. For Supporter 360 and similar, this is the single biggest cost driver — not per-token price, but the round-trip multiplier. Eval should track turns-per-query, not just token spend.