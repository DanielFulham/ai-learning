...
for _var in ("ANTHROPIC_API_KEY", "SERPER_API_KEY"):
    if not os.environ.get(_var):
        raise RuntimeError(
            f"{_var} not set. Copy .env.example to .env and fill it in."
        )
...
# ---------------------------------------------------------------------------
# YAML-loaded agent (via CrewBase)
# ---------------------------------------------------------------------------
...
shopping_task = Task(
    ...
    expected_output=(
        "A GroceryShoppingPlan with total_budget matching input, "
        "one MealPlan carried from upstream, "
        "shopping_sections grouped by StoreSection enum "
        "(Produce/Meat/Dairy/Pantry/Frozen/Bakery/Other) "
        "with each GroceryItem carrying name (bare ingredient, no prep), "
        "quantity (2 lbs / 1 gallon shape), "
        "estimated_price as PriceEstimate {min_dollars, max_dollars}, "
        "and category matching its section. "
        "3-5 shopping_tips."
    ),
...
budget_task = Task(
    ...
    expected_output=(
        ...
        "## Budget Verification — computed total range from the shopping list "
        "vs the requested budget, "
        ...
        "## Substitutions — 2-3 ingredient swaps if the plan exceeds budget, otherwise omit this "
        "section. "
        "Assume upstream shopping list is authoritative; do not re-price items unless suggesting a "
        "substitution."
    ),
